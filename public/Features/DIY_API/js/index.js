...
// Filter jokes based on the query parameters (id, jokeText, and jokeType)
app.get("/filter", (req, res) => {
  const { id, jokeText, jokeType } = req.query;
  const filteredJokes = jokes.filter((joke) => {
    return (
      (!id || joke.id === parseInt(id)) && 
      (!jokeText || joke.jokeText.toLowerCase().includes(jokeText.toLowerCase())) && 
      (!jokeType || joke.jokeType.toLowerCase() === jokeType.toLowerCase())
    );
  });
